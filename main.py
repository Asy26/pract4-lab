...
BOOKS_FILE = "books.md"
BOOK_REGEX = r"####\s+(?P<position>\d+)\.\s+\[(?P<book>.+?)\]\((?P<book_url>.+?)\)\s+by\s+(?P<author>.+?)\s+\((?P<recommended>\d{1,3}\.\d+%)\s\w+\)\s+!\[]\((?P<cover_url>.+?)\)\s+\"(?P<description>.+?)\"\s+\[.+?\]\(.+?\)"     # TODO записать ругулярное выражения для поиска книги
